		...
		if (e < r->elto)  									// quitar e del subárbol izqdo.
			...
			if (r->elto < e)								// quitar e del subárbol drcho.
				...
				if (r->izq.r == 0 && r->der.r == 0) { 		// 1. Raíz es hoja
					...
					if (r->der.r == 0) { 					// 2. Raíz sólo tiene hijo izqdo.
						arbol* a = r->izq.r;
						...
						else								// 4. Raíz tiene dos hijos. Eliminar el mínimo del subárbol derecho y sustituir el elemento de la raíz por éste.
...
template <typename T>
T Abb<T>::borrarMin()							// Elimina el nodo que almacena el menor elemento del árbol. Devuelve el elemento del nodo eliminado.
{
...
//	Método P4
template <typename T>
void Abb<T>::eliminarSubA(const T& e)
{
