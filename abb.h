...
template <typename T> class Abb {
    ...
    public:
        ...
        operator Abin<T>() const;                    // conversión en Abin<T>, esta parece ser la forma de hacerlo.
        ~Abb();
        void poda(const T& e);
   private:
    struct celda {
    ...
    if (r != NODO_NULO){ // árbol no vacío
      if (e == r->elto) { // quitar elemento de la raíz
        if (r->hizq == NODO_NULO && r->hder == NODO_NULO) {//1.Raíz es hoja
          ...
          if (r->hder == NODO_NULO) { // 2.Raíz sólo tiene hijo izqdo.
            nodo n = r->hizq;
            delete(r);
            r = n; // nueva raíz el antiguo hijo izqdo.
          }
          else
             if (r->hizq == NODO_NULO) { // 3.Raíz sólo tiene hijo drcho.
               nodo n = r->hder;
               delete(r);
           ...
           if (e < r->elto) {//quitar elemento del subárbol izqdo.
             Abb<T> Ai(r->hizq);
             Ai.eliminar(e);
             r->hizq = Ai.r;
             Ai.r = NODO_NULO; // impide destruir el subárbol izqdo.
